...
const resolvers = {
  Query: {
    ...
    // users: (parent, args, context, info) => {return User.findOne(name:args.name)},
  },
  Mutation: {
    signup: async (parent, args, context, info) => {
      //username ไม่ควรมี space
      const username = args.username.trim();

      //username ซ้ำสมัครไม่ได้
      const currentUser = await User.find({});
      const isUsernameExist =
        currentUser.findIndex((user) => user.username === username) > -1;
